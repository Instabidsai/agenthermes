...
function SocialIcon({ label, href, children }: { label: string; href: string; children: React.ReactNode }) {
  return (
    <a
      ...
      aria-label={label}
      className="flex h-9 w-9 items-center justify-center rounded-lg bg-zinc-800/60 border border-zinc-700/50 text-zinc-500 hover:text-zinc-200 hover:border-zinc-600 hover:bg-zinc-800 transition-colors"
    >
      {children}
    </a>
  )
  ...
  return (
    <footer className="border-t border-zinc-800/80 bg-[#09090b]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-10 sm:gap-8">
          {/* Brand */}
          <div className="col-span-1 sm:col-span-2">
            ...
            <div className="flex items-center gap-2.5">
              <SocialIcon label="Twitter / X" href="https://x.com/agenthermes">
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </SocialIcon>
              <SocialIcon label="GitHub" href="https://github.com/Instabidsai/agenthermes">
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
              ...
              <SocialIcon label="LinkedIn" href="https://linkedin.com/company/agenthermes">
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
              ...
              <ul className="space-y-2.5">
                {group.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-zinc-500 hover:text-zinc-200 hover:underline decoration-zinc-600 underline-offset-4 transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
